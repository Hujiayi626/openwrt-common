...
if [[ ${SOURCE_CODE} == "LEDE" ]]; then
  ...
  export REPO_URL="https://github.com/immortalwrt/immortalwrt"
  export REPO_BRANCH="openwrt-18.06-k5.4"
else
...
if [[ ! ${bendi_script} == "1" ]]; then
  ...
  echo "USE_CACHEWRTBUILD2=${USE_CACHEWRTBUILD2}" >> ${GITHUB_ENV}
...
if [[ $? -ne 0 ]]; then
  wget -P ${BASE_PATH}/etc https://raw.githubusercontent.com/Hujiayi626/openwrt-common/main/Custom/FinishIng.sh -O ${BASE_PATH}/etc/FinishIng.sh
fi
chmod 775 ${BASE_PATH}/etc/FinishIng.sh
curl -fsSL https://raw.githubusercontent.com/Hujiayi626/openwrt-common/main/Custom/FinishIng > ${BASE_PATH}/etc/init.d/FinishIng
if [[ $? -ne 0 ]]; then
  wget -P ${BASE_PATH}/etc/init.d https://raw.githubusercontent.com/Hujiayi626/openwrt-common/main/Custom/FinishIng -O ${BASE_PATH}/etc/init.d/FinishIng
fi
chmod 775 ${BASE_PATH}/etc/init.d/FinishIng
curl -fsSL https://raw.githubusercontent.com/Hujiayi626/openwrt-common/main/Custom/webweb.sh > ${BASE_PATH}/etc/webweb.sh
if [[ $? -ne 0 ]]; then
  wget -P ${BASE_PATH}/etc https://raw.githubusercontent.com/Hujiayi626/openwrt-common/main/Custom/webweb.sh -O ${BASE_PATH}/etc/webweb.sh
fi
